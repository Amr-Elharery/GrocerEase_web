...
export default function ShopsList() {
  ...
  return (
    <section className="mx-auto max-w-[1420px] space-y-2.5">
      <div>
        <h1 className="text-[24px] font-bold tracking-tight text-[#101828]">{t("shop:shopsList.title")}</h1>
        <p className="mt-0.5 text-[15px] text-[#667085]">{t("shop:shopsList.subtitle")}</p>
      </div>

      {/* Search */}
      <div className="relative max-w-md">
        <Search className="absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#5F7168]" />
        <Input value={search} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearch(e.target.value)}
          placeholder={t("shop:shopsList.searchPlaceholder")} className="h-10 ps-9" />
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-[#DDE7DF] bg-white shadow-sm">
        <div className="grid grid-cols-[1fr_1fr_120px_40px] gap-4 border-b border-[#DDE7DF] bg-[#F8FAF8] px-4 py-3 text-[11px] font-semibold uppercase tracking-wider text-[#667085]">
          <span>{t("shop:shopsList.columns.shop")}</span>
          <span>{t("shop:shopsList.columns.address")}</span>
          <span>{t("shop:shopsList.columns.status")}</span>
          <span></span>
        </div>

        {isLoading ? (
          <p className="px-4 py-8 text-center text-sm text-[#667085]">{t("shop:shopsList.loading")}</p>
        ) : shops.length === 0 ? (
          <p className="px-4 py-8 text-center text-sm text-[#667085]">{t("shop:shopsList.empty")}</p>
        ) : (
          shops.map((shop) => (
            <button key={shop.id} type="button"
              onClick={() => navigate(`/app/shops/${shop.id}`)}
              className="grid w-full grid-cols-[1fr_1fr_120px_40px] items-center gap-4 border-b border-[#F3F4F6] px-4 py-3 text-start transition hover:bg-[#F8FAF8]">
              ...
              <span>
                <span className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-semibold ${
                  ...
                  {shop.is_active ? t("shop:shopsList.active") : t("shop:shopsList.inactive")}
                </span>
              </span>
