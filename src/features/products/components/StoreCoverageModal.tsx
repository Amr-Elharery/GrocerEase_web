  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-6" onClick={onClose}>
      <div className="flex max-h-[75vh] w-full max-w-md flex-col overflow-hidden rounded-2xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex shrink-0 items-start justify-between border-b border-[#EEF2EF] px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#667085]">{product?.brand || "Product details"}</p>
            </div>
          </div>
        ...
        {isLoading ? (
          <p className="px-5 py-12 text-center text-sm text-[#667085]">Loading...</p>
        ) : (
          <div className="flex-1 overflow-y-auto px-5 py-4">

            {/* Info grid */}
            <div className="grid grid-cols-2 gap-3">
              <InfoRow icon={<Tag className="h-3.5 w-3.5" />} label="Category" value={product?.category?.category_name ?? "—"} />
              <InfoRow icon={<Layers className="h-3.5 w-3.5" />} label="Sub-Category" value={product?.sub_category?.category_name ?? "—"} />
              <InfoRow icon={<Box className="h-3.5 w-3.5" />} label="Unit / Size" value={product?.unit ?? "—"} />
              <InfoRow icon={<Store className="h-3.5 w-3.5" />} label="Brand" value={product?.brand ?? "—"} />
            </div>

            {/* Description */}
            {product?.description && (
              <div className="mt-4">
                <p className="text-[10px] font-semibold uppercase tracking-wide text-[#667085]">Description</p>
            ...
            {images.length > 0 && (
              <div className="mt-4">
                <p className="text-[10px] font-semibold uppercase tracking-wide text-[#667085]">Images ({images.length})</p>
            ...
            <div className="mt-4">
              ...
              {shops.length === 0 ? (
                <div className="mt-2 flex flex-col items-center gap-2 rounded-xl bg-[#F8FAF8] py-8 text-center">
                  ...
                  <p className="text-sm text-[#667085]">No store carries this product yet.</p>
                </div>
              ) : (
                    ...
                    <div key={i} className="flex items-center gap-3 rounded-xl border border-[#EEF2EF] bg-[#F8FAF8] px-3 py-2.5">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-[#667085]">
                          {s.available_stock ?? 0} in stock · EGP {(s.price ?? 0).toFixed(2)}
                        </p>
                      </div>
