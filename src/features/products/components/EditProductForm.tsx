...
export default function EditProductForm() {
  ...
  const handleBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    ...
    if (name === "product_name") { if (!value) newErrors.product_name = t("form.errors.required"); else delete newErrors.product_name; }
    if (name === "brand") { if (!value) newErrors.brand = t("form.errors.required"); else delete newErrors.brand; }
    if (name === "barcode") { if (!value) newErrors.barcode = t("form.errors.required"); else delete newErrors.barcode; }
    setErrors(newErrors);
  };
  ...
  const handleNavigateAway = () => {
    if (hasChanges && !window.confirm(t("form.edit.leaveConfirm"))) return;
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-[calc(100vh-64px)] items-center justify-center">
        <p className="text-sm text-[#667085]">{t("form.edit.loading")}</p>
      </div>
    );
  }
  ...
  return (
    <section className="mx-auto max-w-[1420px] space-y-3">
      ...
      <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
        <div>
          <h1 className="text-[22px] font-bold tracking-tight text-[#101828]">{t("form.edit.title")}</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" onClick={handleSubmit}
            ...
            {updateProduct.isPending ? t("form.edit.saving") : t("form.edit.save")}
          </Button>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between rounded-xl border border-[#DDE7DF] bg-[#F8FAF8] px-4 py-2.5">
        <div className="flex items-center gap-6 text-xs text-[#667085]">
          <span className="font-semibold text-[#5F7168]">{t("form.edit.idLabel", { id: product?.id })}</span>
        </div>
      ...
      <div className="grid gap-3 xl:grid-cols-[0.82fr_1.8fr]">
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl border border-[#DDE7DF] bg-white p-3.5 shadow-[0_2px_10px_rgba(15,23,42,0.04)]">
            <h2 className="text-[15px] font-bold text-[#101828]">{t("form.category.sectionTitle")}</h2>
            <div className="mt-3 space-y-1.5">
              <Label className="text-xs font-semibold text-[#101828]">{t("form.category.topLevelLabel")}</Label>
              <div className="relative">
                <button type="button" onClick={() => setCategoryOpen(prev => !prev)}
                  className={`flex h-9 w-full items-center justify-between rounded-lg border bg-[#F8FAF8] px-3 text-start text-sm outline-none transition focus:border-[#2D6A4F] focus:ring-2 focus:ring-[#2D6A4F]/10 ${errors.category_id ? "border-red-500" : "border-[#DDE7DF]"}`}>
                  ...
                  <div className="absolute start-0 end-0 top-[42px] z-50 overflow-hidden rounded-lg border border-[#DDE7DF] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.12)]">
                    {categories.map(c => (
                      <button key={c.id} type="button"
                        ...
                        className={`block w-full px-3 py-2 text-start text-sm transition hover:bg-[#EAF7EE] ${formData.category_id === c.id ? "bg-[#EAF7EE] font-semibold text-[#006B22]" : "text-[#101828]"}`}>
                        {c.name}
                      </button>
                    ))}
                  </div>
            ...
            {selectedCategory && (
              <div className="mt-3 space-y-2">
                <Label className="text-xs font-semibold text-[#101828]">{t("form.category.subCategoryLabel")}</Label>
          ...
          <div className="rounded-xl border border-[#DDE7DF] bg-white p-3.5 shadow-[0_2px_10px_rgba(15,23,42,0.04)]">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-[15px] font-bold text-[#101828]">{t("form.assets.sectionTitle")}</h2>
                <p className="mt-0.5 text-xs text-[#667085]">{t("form.assets.helpText")}</p>
              </div>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
              {images.length < 5 && (
                <div onDragOver={e => { e.preventDefault(); setIsDragging(true); }}
                  ...
                  <p className="text-center text-xs font-medium text-[#5F7168]">{t("form.assets.dropText")}</p>
                  <p className="mt-0.5 text-[10px] text-[#667085]">{t("form.assets.fileTypes")}</p>
                  <span className="mt-1 text-[10px] font-bold uppercase tracking-wide text-[#006B22]">{t("form.assets.browseText")}</span>
              ...
              {images.map((img, i) => (
                <div key={i} className="group relative h-24 overflow-hidden rounded-xl border border-[#DDE7DF] bg-[#F8FAF8]">
                  <img src={img.preview} alt="" className="h-full w-full cursor-pointer object-cover" onClick={() => handleSetPrimary(i)} title={t("form.assets.primaryTitle")} />
                  {i === primaryIndex && (
                    <span className="absolute start-2 top-2 rounded-full bg-[#006B22] px-2 py-0.5 text-[10px] font-semibold text-white">{t("form.assets.primaryBadge")}</span>
                  )}
        ...
        {/* Right Column */}
        <div className="rounded-xl border border-[#DDE7DF] bg-white p-3.5 shadow-[0_2px_10px_rgba(15,23,42,0.04)]">
          <h2 className="text-[15px] font-bold text-[#101828]">{t("form.info.sectionTitle")}</h2>
          <div className="mt-3 space-y-3.5">

            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <Label className="text-xs font-semibold text-[#101828]">{t("form.info.nameLabel")}</Label>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs font-semibold text-[#101828]">{t("form.info.brandLabel")}</Label>
              ...
              <div className="space-y-1.5">
                <Label className="text-xs font-semibold text-[#101828]">{t("form.info.unitLabel")}</Label>
                <div className="relative">
                  <button type="button" onClick={() => setUnitOpen(prev => !prev)}
                    className={`flex h-9 w-full items-center justify-between rounded-lg border bg-[#F8FAF8] px-3 text-start text-sm text-[#101828] outline-none transition focus:border-[#2D6A4F] focus:ring-2 focus:ring-[#2D6A4F]/10 ${errors.unit ? "border-red-500" : "border-[#DDE7DF]"}`}>
                    <span className={formData.unit ? "text-[#101828]" : "text-[#667085]"}>{formData.unit ? unitLabel(formData.unit) : t("form.info.unitPlaceholder")}</span>
                    <ChevronDown className="h-4 w-4 text-[#5F7168]" />
                  </button>
                  {unitOpen && (
                    <div className="absolute start-0 end-0 top-[42px] z-50 overflow-hidden rounded-lg border border-[#DDE7DF] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.12)]">
                      {units.map(unit => (
                        <button key={unit} type="button"
                          ...
                          className={`block w-full px-3 py-2 text-start text-sm transition hover:bg-[#EAF7EE] ${formData.unit === unit ? "bg-[#EAF7EE] font-semibold text-[#006B22]" : "text-[#101828]"}`}>
                          {unitLabel(unit)}
                        </button>
                      ))}
                    </div>
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-semibold text-[#101828]">{t("form.info.barcodeLabel")}</Label>
              <div className="relative">
                <Input name="barcode" value={formData.barcode} onChange={handleChange} onBlur={handleBarcodeBlur}
                  className={`h-9 rounded-lg border-[#DDE7DF] bg-[#F8FAF8] pe-10 text-sm ${barcodeConflict || errors.barcode ? "border-red-500" : ""}`} />
                {barcodeConflict && <AlertTriangle className="absolute end-3 top-1/2 h-4 w-4 -translate-y-1/2 text-red-600" />}
              </div>
              {barcodeConflict && (
                <div className="mt-1 rounded-lg border border-red-200 bg-red-50 p-2.5">
                  <p className="text-xs font-semibold text-red-600">{t("form.info.barcodeConflictTitle")}</p>
                  <p className="mt-0.5 text-xs text-red-600">
                    <Trans
                      i18nKey="products:form.info.barcodeConflictMessage"
                      values={{ barcode: formData.barcode }}
                      components={{ 1: <span className="font-mono font-bold" /> }}
                    />
                  </p>
                </div>
              )}
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-semibold text-[#101828]">{t("form.info.descriptionLabel")}</Label>
